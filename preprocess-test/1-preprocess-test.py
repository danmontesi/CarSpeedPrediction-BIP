...
def preprocess():
    ...
    events_df = pd.read_csv("../bip_assignment/test/events_test.csv")
    ...
    events_df['APPROX_TIME'] = events_df['START_DATETIME_UTC'].dt.ceil('15min')





    speeds_df = pd.read_csv("../bip_assignment/test/speeds_test.csv", usecols=['KEY', 'KM'])
    ...
    merged = merged.merge(road_df, on=["KEY", "KM"])
...
#
    ...
    merged = merged.merge(weather_list, on=["KEY", "KM"])

    #WEATHER
    print("WEATHER")
    weather_df = pd.read_csv("../bip_assignment/test/weather_test.csv")
    ...
    dataset_orig = pd.read_csv("../bip_assignment/test/speeds_test.csv")
