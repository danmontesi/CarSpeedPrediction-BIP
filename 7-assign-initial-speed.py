...
def preprocess():
    ...
    dataset['PREC_SPEED'] = -1

    grouped_dataset = dataset[dataset['READ_INSTANT'] == 1].groupby('KEY')

    for name, group in tqdm(grouped_dataset):
        reduced_orig = dataset_orig[dataset_orig['KEY'] == name]

        for i in tqdm(group.index):
            interesting = reduced_orig[(reduced_orig['KM'] == group.at[i,'KM']) & (reduced_orig['DATETIME_UTC'] == group.at[i,'DATETIME_UTC'] - timedelta(minutes=15))]
...
#
    ...
    dataset.to_csv("bip_assignment/dataset_6.csv", index=False)
